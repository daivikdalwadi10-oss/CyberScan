...
function filterDataByRole(role, data) {
    if (role === 'PublicVisitor')
        return { uptime: data.uptime, system: data.metrics };
    if (role === 'SOCAnalyst')
        return { alerts: data.alerts, threats: data.threats };
    if (role === 'InfraAdmin')
        return { metrics: data.metrics, uptime: data.uptime };
    if (role === 'SuperAdmin')
        return data;
    return {};
}
io.on('connection', async (socket) => {
    ...
    const role = user?.roles?.[0] || 'PublicVisitor';
    const data = {
        metrics: getMetrics(),
        uptime: getUptime(),
